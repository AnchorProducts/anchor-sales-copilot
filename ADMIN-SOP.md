...
**Who keeps the schedule.** Some people on the list get the whole schedule on the showcase page instead of just their own stops. From it they can fix a stop's date, city, event and note, add photos to any stop, remove photos that aren't live, and remove stops. That's decided by the **website**, not here. A person keeps the schedule if their `portal_invites` row has the **Showcase** flag (anchorp.com → Marketing → Authorized users), **or** their role is `admin`, **or** their team is `marketing`. The app asks the website when the page opens, so the rule lives in one place. Keepers still have to be on this list to open the page at all.

Keepers can't publish, and there's no publish button anywhere in the app. Saving a published stop changes the website immediately. Photos they add wait for marketing, and a photo that's already live can't be removed from the app.

**Photos.** A stop can have several photos, up to 10 added at a time, and marketing reviews each one on the website. On a pending stop, **Publish** puts the stop up with every photo still on it, so discard the ones that shouldn't go first. On a stop that's already up, each photo is published or discarded on its own. Every decline reason, for a stop or a single photo, shows to the person who filed it. On the public showcase page, a stop's live photos show as thumbnails under it.

> **Database step:** multiple photos need `scripts/showcase-photos.sql` from the website repo, run once in the Supabase SQL editor. Until it runs, stops load with no photos and adding photos fails.

> **HEIC:** the app converts every photo to JPEG before uploading, and the website refuses HEIC uploads. Publishing copies the file unchanged, and Chrome, Firefox and Android can't show HEIC.
