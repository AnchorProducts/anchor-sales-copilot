// The "logic" behind the Rooftop Equipment Audit (/rooftop): the OSHA decision
// tree that drives every question, branch, and ⚠️ flag is just the system prompt
// sent to the model. This module is the single source of truth for the DEFAULT
// prompt, plus the helpers to render it at request time.
//
// Admins can override this default from /admin/rooftop-logic; the override is
// stored in public.rooftop_assessment_config and read by /api/rooftop. When no
// override exists (or the service key is missing), the route falls back to
// DEFAULT_ROOFTOP_SYSTEM_PROMPT below so the audit always works.
//
// The contractor's name/company isn't known until runtime, so the editable
// template carries a token, CONTRACTOR_IDENTITY_TOKEN, which the route replaces
// with a one-line identity statement before sending to the model. Keep the token
// in the prompt or the assistant won't know who it's greeting.

export const CONTRACTOR_IDENTITY_TOKEN = "{{CONTRACTOR_IDENTITY}}";

export const DEFAULT_ROOFTOP_SYSTEM_PROMPT = `
...
CONTRACTOR IDENTITY: ${CONTRACTOR_IDENTITY_TOKEN}
...
// The one-line contractor identity statement that replaces the token at runtime.
export function contractorIdentityLine(contractorName: string, companyName: string): string {
  const name = (contractorName || "").trim();
  const company = (companyName || "").trim();
  if (name) {
    return `The contractor on this session is ${name}${company ? ` from ${company}` : ""}. Use this exact name when greeting them. Do NOT substitute any other name.`;
  }
  if (company) {
    return `The contractor on this session is from ${company}. No personal name is available; greet them by company name.`;
  }
  return "No contractor name or company is available. Greet them generically (e.g. 'Hello!').";
}

// Render the editable template into the final system prompt for a request.
export function renderRooftopSystemPrompt(
  template: string,
  contractorName: string,
  companyName: string
): string {
  const base = (template || "").trim() || DEFAULT_ROOFTOP_SYSTEM_PROMPT;
  return base.split(CONTRACTOR_IDENTITY_TOKEN).join(contractorIdentityLine(contractorName, companyName));
}
